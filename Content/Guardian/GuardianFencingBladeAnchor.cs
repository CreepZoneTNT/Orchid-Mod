...
public class GuardianFencingBladeAnchor : OrchidModGuardianParryAnchor
{
		...
		else
		{
			...
			if (Projectile.ai[0] > 1) // Charged attack and deflect
			{
				...
				else if (Projectile.ai[0].Between(21f, 61f)) // Reinforced attack state
				{
					...
					DoSlashStyle(GuardianItem.ReinforcedSwingStyle, Projectile.ai[0] - 21f, GuardianItem.SwingSound);
					...
					if (DashInput && GuardianItem.PreDash(Owner, Guardian, Projectile)) // Perform dash attack on right click while reinforced attacking
					{
						Projectile.ai[0] = 20f;
				...
				else if (Projectile.ai[0] <= 20f)
				{
					...
					Owner.position = Collision.TileCollision(Owner.position, Vector2.UnitY.RotatedBy(rotation) * GuardianItem.DashVelocity, Owner.width, Owner.height, true, true, (int)Owner.gravDir);
			...
			else if (Projectile.ai[0] < -1f)
			{
				if (Projectile.ai[0].Between(-41f, -1f))
				{
					if (Projectile.ai[0] == -41f)
					{
						...
						if (GuardianItem.OnSwing(Owner, Guardian, Projectile, false, ref damage))
					...
					float rotation = Projectile.ai[2] - Owner.direction * MathHelper.Pi / 4f * MathF.Sin(0.1571f * (Projectile.ai[0] + 1));
					Projectile.rotation = rotation + MathHelper.Pi;
					Owner.SetCompositeArmFront(true, Player.CompositeArmStretchAmount.Full, rotation);
					Projectile.Center = Owner.GetFrontHandPosition(Player.CompositeArmStretchAmount.Full, rotation) + Vector2.UnitY.RotatedBy(rotation) * BladeTexture.Height * GuardianItem.HoldOffset;
			...
			else if (Projectile.ai[0] == 1)
			{
				...
				Guardian.GuardianItemCharge += 40f / FencingBladeItem.useTime * (Owner.GetTotalAttackSpeed(DamageClass.Melee) * 2f - 1f) * GuardianItem.ChargeRate;
	...
	public void DoSlashStyle(int style, float ai, SoundStyle sound)
	{
		Vector2 sheathPos = Owner.MountedCenter.Floor(); // Sheaths are technically optional, so set a default value in case
		if (SheathTexture != null) // If the sheath texture is present and enabled, set the shea
		{
		...
		switch (style)
		{
			...
			case 1: // Reinforced attack:
				...
				float interval = 40f / (GuardianItem.SwingsPerAttack + 1);
				...
				{
					rotation = Projectile.ai[2] + Owner.direction * MathHelper.Pi / 5f * MathF.Sin((GuardianItem.SwingsPerAttack + 1) * 0.1571f * ai);
				}
				
				if ((int)((40 - ai) / interval) > DamageReset)
				{
					int damage = Guardian.GetGuardianDamage(FencingBladeItem.damage * GuardianItem.ReinforcedSwingDamage);
					if (GuardianItem.OnSwing(Owner, Guardian, Projectile, true, ref damage))
						CreateSlashProj(Vector2.UnitY.RotatedBy(Projectile.ai[2]) * FencingBladeItem.shootSpeed * GuardianItem.ReinforcedSwingVelocity, damage, true);
							
					SoundEngine.PlaySound(sound, Projectile.Center);
					GuardianItem.OnStartAttack(Owner, Guardian, Projectile, true, DamageReset == 0);
					DamageReset++;
					SwingTimer = 0;
				}
				break;
	...
	public void CreateFocusProj(Vector2 velocity, int damage, bool reinforced = false)
	{
		...
		if (newProj.ModProjectile is FencingBladeSlashProjectile slashProj) // Code modified from GuardianKatarAnchor.cs 
		{
			slashProj.FencingBladeItem = GuardianItem;
			slashProj.Strong = reinforced;
			newProj.ai[0] = GuardianItem.SwingsPerAttack;
			newProj.rotation = newProj.velocity.ToRotation();
			newProj.damage = damage;
			newProj.CritChance = (int)(Owner.GetCritChance<GuardianDamageClass>() + Owner.GetCritChance<GenericDamageClass>() + FencingBladeItem.crit);
			newProj.knockBack = FencingBladeItem.knockBack;
